...
test('Recommendation fixture run increments the run counter', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: /Recommendation Agent/ }).click();
  await expect(page.getByRole('heading', { name: 'Recommendation Agent Replay' })).toBeVisible();
...
test('Diagnostic fixture run increments the run counter', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: /Diagnostic Investigation Agent/ }).click();
  await expect(page.getByRole('heading', { name: 'Diagnostic Investigation Replay' })).toBeVisible();
