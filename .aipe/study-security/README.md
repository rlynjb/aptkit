...
This guide traces that axis across the one boundary that actually exists in
AptKit and is blunt about the lenses that don't yet apply.
...
1. **Model output** comes back from the provider and flows into the agent's
   own logic. The model is the untrusted party inside the loop.
2. **API keys** live on the developer's machine and must stay server-side —
   never in the browser bundle, never in a committed artifact.
...
2. **Tool-policy is enforced by omission, not by a second gate** — the
   ...
   the schema-filtering layer. See `01-tool-policy-enforcement-by-omission.md`.
3. **`rubric-improvement` holds the widest grant** — it's the only agent
   whose allowlist includes a mutating tool (`save_judgment`). Every other
   agent is read-only. See `audit.md` → LLM/agent security.
4. **Raw user question into the prompt, no sanitization** — the query agent
   passes the question straight through as the user turn. Prompt-injection
   surface, mitigated only by read-only tools and output validation. See
   `audit.md` → input-validation, and `04-validated-model-output-gate.md`.
