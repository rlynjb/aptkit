...
```
  ...
  │  ★ THIS FILE: the map of these boxes and their one exit ★    │ ← we are here
  └───────────────────────────┬──────────────────────────────────┘
                              │  ModelProvider.complete()
  ┌─ Provider boundary ───────▼──────────────────────────────────┐
  │  Anthropic / OpenAI / Fallback / ContextGuard adapters        │
  └───────────────────────────┬──────────────────────────────────┘
                              │  HTTPS  ◄── the only node-to-node hop
  ┌─ External (not your process) ─▼──────────────────────────────┐
...
AptKit has exactly **two** nodes: your process, and the provider's API. The
Studio UI isn't a separate node in any distributed sense — it calls into the
same process via Vite middleware. The map is two boxes and one arrow. The whole
discipline is about what happens to that arrow.
...
your process is one node, the provider API is the other, and `complete()` is the
edge.
...
```
  The map as a graph — node = failure unit, edge = message
         ...
         ┌─────────────────┐   complete(request)   ┌──────────────────┐
         │  AptKit process │ ────────────────────► │  Provider API    │
         │  (node A)       │ ◄──────────────────── │  (node B)        │
         └─────────────────┘   ModelResponse        └──────────────────┘
              you own            (or it never           someone else owns
              this node           comes back)            this node

  one edge. that's the system.
```

The thing that makes it a *distributed* map rather than two function calls:
edge B can fail independently of node A. Node A keeps running while node B is
down. There's no shared memory, no shared clock, no transaction spanning both.

### Move 2 — walking the map

**The failure domains.** A failure domain is the set of things that go down
together. In AptKit there are two, and they're cleanly separated:

```
  Failure domains — what dies together

  ┌─ Domain 1: your process ──────────┐   ┌─ Domain 2: provider ──────────┐
  │  agent loop, tools, parsing,      │   │  the model API                 │
  │  ledger, Studio                   │   │                                │
  │  dies → nothing remote affected   │   │  dies → your complete() throws,│
  │                                   │   │  loop falls over to fallback   │
  └───────────────────────────────────┘   └────────────────────────────────┘
```

When domain 2 fails, domain 1 stays up and *handles* it — that's the whole
point of the fallback chain. When domain 1 fails (your process crashes), there's
nothing to coordinate; you just restart and re-run. No partial state is stranded
in a remote system because there's no remote state in this design.
...
`artifacts/replays/`. The provider owns *nothing* of yours between calls — each
`complete()` is stateless from your side; you re-send the full message history
every turn. That stateless-per-call design is why there's no consistency
problem to solve (covered in `04`).

**The messages.** Exactly one message type crosses the boundary: a
`ModelRequest` out, a `ModelResponse` back (or an error / nothing). There's no
gossip protocol, no heartbeat, no replication stream. The richness lives
*inside* domain 1 as `CapabilityEvent`s on a trace, never on the wire.

### Move 3 — the principle

The most senior thing you can say about a system is **"here's the smallest
true map of it."** For AptKit that map is two nodes and one edge. Drawing it
honestly — rather than inventing services that don't exist — is the skill. A
system with one network edge needs the discipline of *that one edge* and
nothing more; bolting on consensus or queues would be complexity with no
failure to justify it.

## Primary diagram

The full recap — every layer, the two nodes, the one edge, and where failure
is contained.
...
```
  ...
  │  state lives HERE: messages[], toolCalls[], events[], artifacts │
  └────────────────────────────┬───────────────────────────────────┘
                              │ ModelProvider.complete()  ← THE SEAM
                              │   (failure axis flips here)
  ┌─ Provider adapters (still NODE A) ─▼───────────────────────────┐
  │  Anthropic · OpenAI · Fallback (chain) · ContextGuard (gate)    │
  └────────────────────────────┬───────────────────────────────────┘
                              │ HTTPS  ◄── THE ONLY node-to-node edge
  ┌─ External (NODE B — provider, failure domain 2) ─▼─────────────┐
  │  api.anthropic.com         api.openai.com                       │
...
differ in prompt and tool policy, not topology.
...
- Node B (provider): reached only through an adapter, e.g.
  `packages/providers/anthropic/src/anthropic-provider.ts:28-61`, which is the
  thinnest possible translation layer over `client.messages.create(...)`.

**What's NOT on the map (and the trigger that would add it):**

- No second process, no IPC, no RPC between your own components →
  *trigger: extracting the agent loop into its own service that Studio calls
  over HTTP.*
- No datastore node → *trigger: persisting artifacts to a remote DB instead of
...
Draw the two-node graph. Say: "It's single-process by design — a library plus a
local dev tool. There's exactly one distributed edge: the call to an external
model provider over HTTPS. Everything else is in-process function calls."

```
  [your process] ──complete()──► [provider API]
   owns all state              owns nothing of yours
   failure domain 1            failure domain 2
```

Anchor: "The seam is `ModelProvider.complete()` — that's where the failure axis
flips from in-process exceptions to partial network failure."
...
1. **Reconstruct:** Draw AptKit's full map from memory — how many nodes, how
   many edges, where's the seam?
2. **Explain:** Why is `apps/studio` *not* a separate node in the distributed
   sense, even though it has its own API routes?
