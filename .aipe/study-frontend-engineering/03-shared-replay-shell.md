...
Five of the six workspaces are shell adapters. The clearest is `RecommendationWorkspace` (`RecommendationWorkspace.tsx:23-47`): pure configuration, no state of its own at the top level except a `resetToken` it bumps on fixture/mode change to reset child panels. `RubricImprovementWorkspace` is the leanest adapter (`RubricImprovementWorkspace.tsx:17-37`) — it doesn't even need a `resetToken` because it has no save/compare panels. The sixth workspace, `CapabilitiesWorkspace`, deliberately does *not* use the shell — it runs four utility previews in `Promise.all` rather than one agent replay, so it has its own bespoke layout (`CapabilitiesWorkspace.tsx:102-220`). That asymmetry is correct: forcing it into `<F, M, R>` would be abstraction theater.
